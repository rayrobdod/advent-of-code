...
case class PathfindState(cost: Int, position: Point, previousMove: Direction, previousMoveRepeat: Int):
	...
	def allowedNextMoves: Set[Direction] =
		if 0 == previousMoveRepeat then
			Direction.all
		else if previousMoveRepeat >= 3 then
			previousMove.turns
		else
...
def pathfind(grid: Grid[Int]): Int =
	...
	while toVisit.head.position != target do
		...
		//System.out.print(s"ENTERING: $current $toVisit")
		//new java.util.Scanner(System.in).nextLine()

		seen += current.toSeen

		val allowedMoves = current.allowedNextMoves
